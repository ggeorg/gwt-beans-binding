...
public class BeanPropertyDescriptorGenerator extends Generator {
  ...
  private void write(SourceWriter w, JClassType _type, JClassType[] types) {
    ...
    w.println("static {");
    w.indent();
    w.println("try{setupBeanInfo();}catch(Exception ex){com.google.gwt.user.client.Window.alert(ex.getMessage());}");
    w.outdent();
    w.println("}");
  }
  ...
  private void writePropertyDescriptor(SourceWriter sw, JClassType type,
    ...
    if (getter != null) {
      ...
      sw.println("{");
      sw.indent();
      sw.println("return ( (" + type.getQualifiedSourceName() + ") bean)."
          + getter.getName() + "();");
      sw.outdent();
      sw.println("}");
      sw.outdent();
