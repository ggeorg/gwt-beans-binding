 ...
 * Each object in the source {@code List} provides one element in the
 * {@code ListBox}. By setting a
 ...
 * corresponding object in the source {@code List}. The default
 * {@code DetailBinding} uses the objects directly. Instances of
 * {@code ListBoxBinding} are obtained by calling one of the
 * {@code createListBoxBinding} methods in the {@code SwingBindings} class.
 * <p>
 * Here is an example of creating a binding from a {@code List} of
 * {@code Person} objects to a {@code ListBox}:
 * <p>
 * 
 * <pre><code>
 ...
 * {@code List} of objects to show in the target {@code ListBox}. This
 * synthetic property is readable/writeable only when the {@code ListBoxBinding}
 * is bound and the target {@code ListBox} property is readable with a
 * {@code non-null} value.
 * <p>
 ...
 * target {@code ListBox}). These methods do not, therefore, have anything to
 * do with refreshing <i>values</i> in the {@code ListBox}. Likewise, the
 * update strategy, which simply controls when {@code refresh} and {@code save}
 * are automatically called, also has nothing to do with refreshing <i>values</i>
 * in the {@code ListBox}.
 * <p>
 * <b>Note:</b> At the current time, the {@code READ_WRITE} update strategy is
 * not useful for {@code ListBoxBinding}. To prevent unwanted confusion,
 * {@code READ_WRITE} is translated to {@code READ} by {@code ListBoxBinding's}
 * constructor.
 * <p>
 ...
 * model is uninstalled from a target {@code ListBox} when either the
 * {@code ListBoxBinding} is unbound or when the target {@code ListBox} property
 * changes to no longer represent that {@code ListBox}. Note: When the model is
 * uninstalled from a {@code ListBox}, it is replaced with a
 * {@code DefaultListModel}, in order to leave the {@code ListBox} functional.
 * <p>
 ...
 * property representing a {@code ListBox} ({@code jListP}). {@code listP} and
 * {@code jListP} both start off readable, referring to a {@code non-null}
 * {@code List} and {@code non-null} {@code ListBox} respectively. Let's look at
 ...
 * {@code ListBox}, representing list of objects </td>
 * </tr>
 * <tr valign="baseline">
 ...
 * <p>
 * {@code DetailBindings} are managed by the {@code ListBox}. They are not to
 * be explicitly bound, unbound, added to a {@code BindingGroup}, or accessed
 ...
 * a {@code ListBox} backed by a {@code ListBoxBinding}, the selection is
 * always in terms of elements from the source {@code List}, regardless of any
 * {@code DetailBinding} specified. See the list of <a
...
public final class ListBoxBinding<E, SS, TS> extends
  ...
  private final class BindingListModel extends ListBindingManager {
    ...
    public BindingListModel() {
      // XXX listeners = new CopyOnWriteArrayList<ListDataListener>();
    }
    ...
    private void contentsChanged(int row0, int row1) {
      ...
      // }
      for (int i = Math.min(row0, row1), n = Math.max(row0, row1); i < n; ++i) {
        if (i < list.getItemCount()) {
          list.insertItem(getElementAt(i), i);
        } else {
